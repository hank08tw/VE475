#include <iostream>
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <memory.h>
using namespace std;
void generate()
{
    char alphabet[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789,.;?!()";
    srand(time(NULL));
    //string tmp;
    for (int i=0;i<=15;i++){
        //str.push_back(alphabet[rand() % 69]);
        cout << alphabet[rand()%69];
    }
    cout << endl;
    //return str;
}
void swap_two(uint8_t *first, uint8_t *second)
{
    uint8_t tmp = *first;
    *first = *second;
    *second = tmp;
}
// sbox is copied from slide, invsbox is derived from sbox, rcon is copied from wiki.
uint8_t rcon[256] = {};

uint8_t sbox[256] = {};

uint8_t invsbox[256] = {};

#define mult_funct(tmp) (uint8_t)((tmp > 127) ? (27^(tmp << 1)) : ((tmp) << 1))

uint8_t mult(uint8_t first, uint8_t second){
    if (1 & second){return first ^ mult_funct(mult(first, second >> 1));}
    else if (1==second){return first;}
    else if (2==second){return mult_funct(first);}
    else return mult_funct(mult(first, second >> 1));
}

void AddRoundKey(uint8_t input[16], uint8_t private_key[16]){
    for(int i=0;i<=15;i++){
        input[i] = input[i] ^ private_key[i];
    }
}

uint8_t mult_funct2(uint8_t tmp,int n){
    uint8_t tmp2=tmp;
    for(int i=0;i<n-1;i++){
        tmp2=tmp2^tmp;
    }
    return tmp2;
}

#define MixColumnCalculation(input) for(int i=0;i<=3;i++){tmp[i] = byte[i*4 +input];} tmp[4]=tmp[0]^tmp[1]^tmp[2]^tmp[3];for(int i=0;i<=3;i++){byte[i*4+input]=tmp[i] ^ mult_funct2(tmp[i] ^ tmp[i+1],2) ^ tmp[4];}
void MixColumns(uint8_t byte[16]){
    uint8_t tmp[5];
    for(int i=0;i<=3;i++){
        MixColumnCalculation(i)
    }
}
#define InvMixColumnCalculation(input) for(int i=0;i<=3;i++){tmp[i] = byte[i*4 +input];} tmp[4]=tmp[0]^tmp[1]^tmp[2]^tmp[3];for(int i=0;i<=3;i++){byte[i*4+input]=tmp[(i+3)%4] ^ mult_funct2(tmp[(i+1)%4],3) ^ mult_funct2(tmp[(i+2)%4],5) ^ mult_funct2(tmp[i],6) ^ mult_funct2(tmp[4],8);}
void InvColumns(uint8_t byte[16]){
    uint8_t tmp[5];
    for(int i=0;i<=3;i++){
        InvMixColumnCalculation(i);
    }
}


void ShiftRows(uint8_t input[16]){
    uint8_t tmp = input[15];
    input[15] = input[14];
    input[14] = input[13];
    input[13] = input[12];
    input[12] = tmp;
    tmp = input[4];
    input[4] = input[5];
    input[5] = input[6];
    input[6] = input[7];
    input[7] = tmp;
    swap_two(&input[11], &input[9]);
    swap_two(&input[10], &input[8]);

}

void InvShiftRows(uint8_t input[0x10]){
    uint8_t tmp = input[12];
    input[12] = input[13];
    input[13] = input[14];
    input[14] = input[15];
    input[15] = tmp;
    tmp = input[7];
    input[7] = input[6];
    input[6] = input[5];
    input[5] = input[4];
    input[4] = tmp;
    swap_two(&input[11], &input[9]);
    swap_two(&input[10], &input[8]);
}

void SubBytes(uint8_t input[16]){
    for(int i=0;i<=15;i++){
        input[i] = sbox[input[i]];
    }
}

void InvSubBytes(uint8_t input[16]){
    for(int i=0;i<=15;i++){
        input[i] = invsbox[input[i]];
    }
}

uint8_t *to_byte(uint8_t byte[16]){
    uint8_t *tmp = (uint8_t*)malloc(sizeof(uint8_t)*16);
    //uint8_t *tmp = new uint8_t[16];
    for (int i = 0; i < 4; i++){
        for (int j = 0; j < 4; j++){
            tmp[j+i * 4] = byte[i + j * 4];
        }
    }
    return tmp;
}
uint8_t *encrypt(uint8_t* plaintext, uint8_t private_key[16]){
    uint8_t *byte = to_byte(plaintext);
    uint8_t **myroundkey=(uint8_t**)malloc(sizeof(uint8_t *)*11);
    //uint8_t **myroundkey=new (uint8_t*);
    myroundkey[0] = to_byte(private_key);
    for (int i = 1; i <= 10; i++){
        myroundkey[i] = (uint8_t*)malloc(sizeof(uint8_t)*16);
        //myroundkey[i] = new uint8_t[16];
    }
    for (int i = 1; i <= 10; i++){
        myroundkey[i][8] = sbox[myroundkey[i - 1][15]] ^ (uint8_t) (0) ^ myroundkey[i - 1][8];
        myroundkey[i][12] = sbox[myroundkey[i - 1][3]] ^ (uint8_t) (0) ^ myroundkey[i - 1][12];
        myroundkey[i][0] = sbox[myroundkey[i - 1][7]] ^  myroundkey[i - 1][0] ^ rcon[i];
        myroundkey[i][4] = sbox[myroundkey[i - 1][11]] ^ (uint8_t) (0) ^ myroundkey[i - 1][4];   
        for (int j = 0; j <= 15; j++){
            if (j % 4){myroundkey[i][j]=myroundkey[i - 1][j] ^ myroundkey[i][j - 1];}
        }
    }
    for (int i = 0; i <= 8; i++){
        AddRoundKey(byte, myroundkey[i]);
        SubBytes(byte);
        ShiftRows(byte);
        MixColumns(byte);
    }
    AddRoundKey(byte, myroundkey[9]);
    SubBytes(byte);
    ShiftRows(byte);
    AddRoundKey(byte, myroundkey[10]);
    uint8_t *finalciphertext = to_byte(byte);
    return finalciphertext;
}
/*
uint8_t *encrypt(uint8_t plaintext[16], uint8_t private_key[16]){
    uint8_t *byte = to_byte(plaintext);
    uint8_t **myroundkey=(uint8_t**)malloc(sizeof(uint8_t *)*11);
    //uint8_t **myroundkey=new (uint8_t*);
    myroundkey[0] = to_byte(private_key);
    for (int i = 1; i <= 10; i++){
        myroundkey[i] = (uint8_t*)malloc(sizeof(uint8_t)*16);
        //myroundkey[i] = new uint8_t[16];
    }
    for (int i = 1; i <= 10; i++){
        myroundkey[i][8] = sbox[myroundkey[i - 1][15]] ^ (uint8_t) (0) ^ myroundkey[i - 1][8];
        myroundkey[i][12] = sbox[myroundkey[i - 1][3]] ^ (uint8_t) (0) ^ myroundkey[i - 1][12];
        myroundkey[i][0] = sbox[myroundkey[i - 1][7]] ^  myroundkey[i - 1][0] ^ rcon[i];
        myroundkey[i][4] = sbox[myroundkey[i - 1][11]] ^ (uint8_t) (0) ^ myroundkey[i - 1][4];   
        for (int j = 0; j <= 15; j++){
            if (j % 4){myroundkey[i][j]=myroundkey[i - 1][j] ^ myroundkey[i][j - 1];}
        }
    }
    for (int i = 0; i <= 8; i++){
        AddRoundKey(byte, myroundkey[i]);
        SubBytes(byte);
        ShiftRows(byte);
        MixColumns(byte);
    }
    AddRoundKey(byte, myroundkey[9]);
    SubBytes(byte);
    ShiftRows(byte);
    AddRoundKey(byte, myroundkey[10]);
    uint8_t *finalciphertext = to_byte(byte);
    return finalciphertext;
}
*/

uint8_t *decrypt(uint8_t plaintext[16], uint8_t private_key[16]){
    uint8_t *byte = to_byte(plaintext);
    uint8_t **myroundkey=(uint8_t**)malloc(sizeof(uint8_t *)*11);
    //uint8_t **myroundkey;//=new (*uint8_t)[11];
    myroundkey[0] = to_byte(private_key);
    for (int i = 1; i <= 10; i++){
        myroundkey[i] = (uint8_t*)malloc(sizeof(uint8_t)*16);
        //myroundkey[i] = new uint8_t[16];
    }
    for (int i = 1; i <= 10; i++){
        myroundkey[i][8] = sbox[myroundkey[i - 1][15]] ^ (uint8_t) (0) ^ myroundkey[i - 1][8];
        myroundkey[i][12] = sbox[myroundkey[i - 1][3]] ^ (uint8_t) (0) ^ myroundkey[i - 1][12];
        myroundkey[i][0] = sbox[myroundkey[i - 1][7]] ^  myroundkey[i - 1][0] ^ rcon[i];
        myroundkey[i][4] = sbox[myroundkey[i - 1][11]] ^ (uint8_t) (0) ^ myroundkey[i - 1][4];
        for (int j = 0; j <= 15; j++){
            if (j % 4)myroundkey[i][j]=myroundkey[i - 1][j] ^ myroundkey[i][j - 1];
        }
    }
    AddRoundKey(byte, myroundkey[10]);
    for (int i = 0; i <= 8; i++)
    {
        InvShiftRows(byte);
        InvSubBytes(byte);
        AddRoundKey(byte, myroundkey[9 - i]);
        InvColumns(byte);
    }
    InvShiftRows(byte);
    InvSubBytes(byte);
    AddRoundKey(byte, myroundkey[0]);
    uint8_t *finalplaintext = to_byte(byte);
    return finalplaintext;
}

int main(int argc, char *argv[]){
    
    if (strcmp("--encrypt",argv[1])==0 && strcmp("--key",argv[3]) == 0){cout << encrypt(argv[2], argv[4]) << endl;}
    else if (strcmp("--decrypt", argv[1]) == 0 && strcmp("--key",argv[3]) == 0){cout << decrypt(argv[2], argv[4]) << endl;}
    else if (strcmp(argv[1], "--generate") == 0){/*cout << generate() << endl;*/generate();}
    else{cout << "Wrong input format!!!" << endl;}
    /*
    uint8_t myplaintext[16]={0x74, 0x56, 0xfe, 0x28,0xdd, 0xae, 0xdc, 0x43,0x27, 0x59, 0x21, 0xee,0xca, 0xcd, 0xdc, 0x88};
    uint8_t myprivate_key[16] = {0xff, 0x04, 0xd0, 0xff,0x42, 0x39, 0x21, 0xce,0x64, 0x73, 0xee, 0xbb,0xbf, 0xfb, 0x5b, 0xb3};
    printf("the plaintext is: \n");
    for (int i = 0; i <= 15; i++){
        printf("%0x  ",myplaintext[i]);
    }
    printf("\n");
    printf("the private_key is: \n");
    for (int i = 0; i <= 15; i++){
        printf("%0x  ",myprivate_key[i]);
    }
    printf("\n");
    uint8_t *myciphertext = encrypt(myplaintext, myprivate_key);
    printf("the ciphertext is: \n");
    for (int i = 0; i <= 15; i++){
        printf("%0x  ",myciphertext[i]);
    }
    printf("\n");
    
    uint8_t *myplaintext2 = decrypt(myciphertext, myprivate_key);
    printf("the plaintext decrypts from ciphertext is: \n");
    for (int i = 0; i <= 15; i++){
        printf("%0x  ",myplaintext2[i]);
    }
    printf("\n");
    */
    return 0;
}